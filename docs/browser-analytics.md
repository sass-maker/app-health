...
4. Bind `BROWSER_HISTORY` to R2 without an age-based object lifecycle. Physical
   source objects can be replaced only after verified, count-equivalent
   compaction; logical analytics facts remain. Bind `BROWSER_ANALYTICS` to the
   `app_health_browser_v1` Analytics Engine dataset. Existing account ID and
   query-token bindings must authorize that dataset.
