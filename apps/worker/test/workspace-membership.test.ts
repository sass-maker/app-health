...
describe('workspace membership and project sources (0016)', () => {
  it('applies cleanly after every earlier migration and seeds owner membership', () => {
    const db = migratedDb();
    db.prepare("INSERT INTO \"user\"(id, name, email, emailVerified, image, createdAt, updatedAt) VALUES ('u1', 'Owner', 'o@example.com', 1, NULL, 1, 1)").run();
    db.prepare("INSERT INTO workspaces(id, owner_id, name, created_at) VALUES ('w1', 'u1', 'My workspace', 1)").run();
    db.exec(readFileSync(new URL('../migrations/0016_workspace_membership.sql', import.meta.url), 'utf8'));
  ...
  it('bounds membership roles and source kinds to the documented sets', () => {
    const db = migratedDb();
    db.prepare("INSERT INTO \"user\"(id, name, email, emailVerified, image, createdAt, updatedAt) VALUES ('u1', 'Owner', 'o@example.com', 1, NULL, 1, 1)").run();
    db.prepare("INSERT INTO \"user\"(id, name, email, emailVerified, image, createdAt, updatedAt) VALUES ('u2', 'Member', 'm@example.com', 1, NULL, 1, 1)").run();
    db.prepare("INSERT INTO \"user\"(id, name, email, emailVerified, image, createdAt, updatedAt) VALUES ('u3', 'Other', 'x@example.com', 1, NULL, 1, 1)").run();
    db.prepare("INSERT INTO workspaces(id, owner_id, name, created_at) VALUES ('w1', 'u1', 'My workspace', 1)").run();
    db.exec(readFileSync(new URL('../migrations/0016_workspace_membership.sql', import.meta.url), 'utf8'));

    db.prepare("INSERT INTO workspace_members(workspace_id, user_id, role, created_at) VALUES ('w1', 'u2', 'viewer', 1)").run();
    // Duplicate membership is rejected...
    expect(() =>
      db.prepare("INSERT INTO workspace_members(workspace_id, user_id, role, created_at) VALUES ('w1', 'u2', 'editor', 1)").run(),
    ).toThrow();
    // ...and so is a role outside the documented set.
    expect(() =>
      db.prepare("INSERT INTO workspace_members(workspace_id, user_id, role, created_at) VALUES ('w1', 'u3', 'admin', 1)").run(),
    ).toThrow();

    db.prepare("INSERT INTO apps(id, name, created_at) VALUES ('app1', 'Product', 1)").run();
    db.prepare("INSERT INTO environments(id, app_id, name, created_at) VALUES ('env1', 'app1', 'production', 1)").run();
    db.prepare(
