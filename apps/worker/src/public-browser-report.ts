...
export async function queryPublicBrowserBreakdowns(
  ...
  const [trend, pages, sources, sessions, countries] = await Promise.all([
    ...
    rows(
      `SELECT ${analyticsSourceSql()} AS name, SUM(_sample_interval) AS count, MAX(_sample_interval) AS sample_interval ${analyticsSourceFrom(source + " AND blob3 = 'pageview'")} GROUP BY name ORDER BY count DESC LIMIT 20`,
      options,
    ),
    rows(
