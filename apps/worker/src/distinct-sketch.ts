...
function sparseDecode(value: string): Set<string> {
  ...
  if (hashes.length > SPARSE_LIMIT || hashes.some((hash) => !/^[a-f0-9]{16}$/.test(hash)))
...
export class DistinctSketchAccumulator {
  ...
  addDigest(hash: string): this {
    if (!HASH_PATTERN.test(hash)) throw new Error('distinct input must be a SHA-256 digest');
    if (this.sparse) {
      this.sparse.add(hash.slice(0, 16));
      if (this.sparse.size <= SPARSE_LIMIT) return this;
      this.dense = registersFor(this.precision);
      for (const value of this.sparse) applyHash(this.dense, this.precision, value.padEnd(64, '0'));
      this.sparse = null;
      return this;
    }
  ...
  merge(sketch: SparseDistinctSketch): this {
    ...
    if (sketch.encoding === 'sparse') {
      for (const hash of sparseDecode(sketch.registers)) this.addDigest(hash.padEnd(64, '0'));
      return this;
    }
    ...
    if (this.sparse) {
      ...
      this.sparse = null;
      this.dense = registersFor(this.precision);
      for (const hash of hashes) applyHash(this.dense, this.precision, hash.padEnd(64, '0'));
    }
